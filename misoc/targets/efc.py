...
class _RtioSysCRG(Module, AutoCSR):
    def __init__(self, platform, clk_freq=125e6):
        ...
        gtpclk = platform.request("gtp_clk")
        if clk_freq == 125e6:
            clkmult = 16
            dqsper = 2.0
            gtpclk_div2_period = 16.0
        elif clk_freq == 100e6:
            clkmult = 20
            dqsper = 2.5
            gtpclk_div2_period = 20.0
        else:
            raise ValueError("Only 100 and 125MHz clock frequencies are supported")
        platform.add_period_constraint(gtpclk, gtpclk_div2_period/2)
        self.gtpclk_buf = Signal()
        self.gtpclk_div2 = Signal()
        self.specials += Instance("IBUFDS_GTE2",
            i_CEB=0,
            i_I=gtpclk.p, i_IB=gtpclk.n,
            o_O=self.gtpclk_buf,
            o_ODIV2=self.gtpclk_div2,
            p_CLKCM_CFG="TRUE",
            p_CLKRCV_TRST="TRUE",
            p_CLKSWING_CFG=3)
        if hw_rev == "v1.0":
        ...
        self.specials += [
            Instance("PLLE2_BASE",
                p_CLKIN1_PERIOD=gtpclk_div2_period,
                i_CLKIN1=self.gtpclk_div2,
                ...
                # VCO @ 1GHz
                p_CLKFBOUT_MULT=clkmult, p_DIVCLK_DIVIDE=1,
        ...
        self.specials += [
            Instance("MMCME2_BASE",
                ...
                # VCO @ 1.0GHz or 1.25 GHz with MULT=20
